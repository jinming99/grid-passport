...
## Amendment A-3 (2026-04-19 — pending Ming sign-off)

**Changed (headline):** four post-§17 pre-run fixes surfaced by the step-4 channel-implementation review. All changes apply before the first pilot/main run:

1. **§6a meeting turnaround reconciled to 4d total.** The §6a turnaround *table* bundles "Meeting duration: 1 simulated day (meeting + notes distribution)" into one row; the *Meeting protocol* bullet below split this as "1 day for the meeting itself + 1 day for notes to circulate = 5 simulated days per meeting." These two presentations disagreed by 1 day. The table wins: a complete meeting sequence is **3d scheduling + 1d meeting+notes = 4 simulated days** (matches `TURNAROUND_DAYS_B["meeting_scheduling"] + ["meeting_plus_notes"]` in `eval_sim/config.py`). The redundant `MEETING_DURATION_DAYS = 5.0` constant is removed. `eval_sim/channels/email.py` now advances the two verbal-contribution turns by `TURNAROUND_DAYS_B["meeting_plus_notes"] / 2 = 0.5d` each. §6a bullet text + §16.1 #4 resolved-questions text both updated to match.
2. **§5e row 1 clarified — applicant tier follows the product-agent row per condition.** The original row read "Applicant (contract-handler + technical-expert) — all conditions: Sonnet 4.6." Taken literally this contradicted §4.1 ("Opus 4.7 for Oracle agents — same tier as D's Skill stack") and §5e rows 5–7 (C/D/A product-agent tier = Opus 4.7). The consistent reading that preserves the §4.1 fairness-with-D requirement and the §5d/§14 #9 judge/user-sim self-preference mitigation is: **applicant agent tier follows the per-condition product-agent row** — B = Sonnet 4.6 (no AI intermediary; user-sim role-play); A, C, D = Opus 4.7 (matches the Oracle plan-producer / flat-prompt agent / Skill stack tier). Utility + regulator remain Sonnet 4.6 across all conditions to preserve the 1-tier judge-agent gap. This matches `eval_sim/runner.py` already. API-pricing check: Opus 4.7 ($5/$25 per MTok) is ~1.67× Sonnet 4.6 ($3/$15 per MTok) — not a budget-blocker, but unifying *all* agents to Opus would collapse the judge-vs-agent tier gap and reintroduce the Panickssery 2024 self-preference confound at its uppermost-bound rate, so the tiered design is retained.
3. **§6a vector-3 "verbal-to-text compounding" treated as parallel draws in v1.** The §6a meeting-leak bullet says meetings introduce "a second paraphrase-loss event layered on top of the in-email one" implying compounding loss on the same piece of information. v1's `ParaphrasedPrivateProfile` component runs the paraphrase barrier independently per applicant act (fresh SHA256-seeded draw per incrementing counter); meeting turns produce **parallel paraphrase samples, not compounded loss**. True compounding would require state passing between the email-thread paraphrase and the meeting-notes paraphrase, which v1 does not model. Flagged for §11 confounds table and §14 honest limits.
4. **§6a `scheduler_assistant_cc` fires unconditionally in v1.** The §6a table row for S1 reads "40% (*when exec in thread*)." v1 has no explicit exec persona in the simulation, so `scheduler_assistant_cc` fires on the raw archetype base rate regardless of whether the email thread actually has an exec recipient. This slightly over-samples the leak for S1/S5 in the (common) absence of an exec; any under-sampling in threads that *do* have an exec is not modeled. Flagged for §14 honest limits.

**Reason:** pre-run review of the step-4 channel implementation (5 commits `7541a8d` → `8fe1f4e`, all pre-run) surfaced: (a) the §6a internal inconsistency above that no single code path could satisfy both readings of; (b) a §5e row-1 vs §4.1 literal contradiction that blocked a clean amendment-free implementation; (c) two fidelity simplifications worth documenting rather than silently diverging from the design. Each of (1)–(4) is either a doc-to-code reconciliation (1, 2) or a pre-registered simplification with explicit honest-limits disclosure (3, 4).

**Impact on prior data:** none — no pilot or main runs have initiated. The §17 sign-off covers A-1 + A-2; A-3 applies before the first run so no data is invalidated.
...
- **Turnaround.** 3 simulated days to coordinate schedules + 1 day for meeting + notes distribution = **4 simulated days per meeting** (matches the §6a turnaround table; see Amendment A-3 for the table-vs-bullet reconciliation).
...
4. **Meeting-request protocol.** RESOLVED — triggered after **3 complete email rounds** without resolution; 4 simulated days per meeting (3d schedule + 1d meeting+notes, per §6a table; see Amendment A-3 for the reconciliation); meetings as leak amplifier (3 distinct trace-leak vectors: attendee broadening, notes-doc forwarding, verbal-to-text paraphrase) — see §6a. Meetings are modeled explicitly, not skipped.
...
- **2026-04-19 (Amendment A-3)** — pre-run post-§17 channel-implementation review fixes, all before the first pilot/main run. (1) §6a meeting turnaround reconciled to 4d/meeting (table wins over bullet; `MEETING_DURATION_DAYS` constant removed from `eval_sim/config.py`; `email.py` verbal-turn advances shortened to 0.5d each; §6a bullet + §16.1 #4 text updated). (2) §5e row 1 clarified — applicant tier follows the per-condition product-agent row (B Sonnet; A/C/D Opus; utility+regulator always Sonnet). Pricing check (~1.67× Opus vs Sonnet) noted, but unifying everything to Opus would collapse the §5d/§14 #9 judge-vs-user-sim tier gap and reintroduce the Panickssery 2024 self-preference confound, so the tiered design is retained. (3) §6a vector-3 verbal-to-text compounding treated as parallel `ParaphraseBarrier` draws in v1 (flagged §11/§14). (4) §6a `scheduler_assistant_cc` fires unconditionally in v1 since there is no exec persona (flagged §14). Also shipped in the same commit: `channels/oracle.py` now tags `scorer_inputs["cartographer_mode"] = "cached"` per §6e (previously omitted — §8d H-spec.hallucination decomposition now joins A/D uniformly vs C's "live"); stale "pending step 4X" markers removed from `channels/__init__.py`; `docs/plans/handoff.md` architecture-tree test count synced to 218. No data invalidated — no runs initiated.
