...
**§17 signed off (2026-04-19). Post-lock implementation: steps 1–6a of 7 complete in 8 commits (`b15a395` → `2d09b3d`). Amendment A-3 applied pre-pilot (`ab9ba87`). Step 6a = pilot wiring + 12-subset validation — pilot engine proven end-to-end, step-6 remaining work decomposes into 6b/6c/6d below. 6c + 6d landed 2026-04-20 (code; uncommitted). Amendment A-4 landed 2026-04-20 after the 6d scorer batch surfaced a token-hygiene false-positive class on the 12-subset — see `docs/evals/sim-bench-results.md` "Methodology findings".** The 7-step plan to live-bench:
...
6. **Pilot** — 28 main + 9 fairness-check = 37 runs. Decomposed into:
   ...
   - **6b** — 16+ more main-pilot runs to reach §9 seed coverage (seeds 1–4 for S1–S3 + full 5-seed sweep for S4/S5/S6) = 27+ more Opus-heavy runs, ~2.5–4 hrs wall-clock, ~$40–80 SDK. **Gated on 6d signal analysis; pending.**
   - **6c** — ✅ (2026-04-20; uncommitted) — `runner.run(cartographer_mode_override='live'|'cached'|None)` kwarg threads through `SkillBundleChannel` + `PromptOnlyBundleChannel`. `pilot.py --fairness` flipped from `typer.Exit(1)` stub to real dispatch over `CARTOGRAPHER_FAIRNESS_PILOT_{SCENARIOS,SEEDS}` × C+D with live override. `--fairness-only` skip-main-pilot flag added. 5 new runner tests (override on D / C / A-rejected / B-rejected / invalid-value). Pending: actual 12 fairness-pilot runs (~1–2 hrs wall-clock, ~$20–30 SDK).
   - **6d** — ✅ (2026-04-20; uncommitted) — `scripts/score_ledgers.py` batch: deterministic axes (efficiency, mechanical h_null, direct T1+T2) + LLM-gated axes (direct T3 paraphrase judge, trace classifier per-CI-tuple, Opus Prometheus judge). Idempotent per-cell merge, `--deterministic-only` / `--with-llm` / `--scorer <name>` / `--force` flags, per-scorer budget tracking (calls + token estimate + wall). `--token-source scenario|ledger` to let amendments apply retroactively to historical ledgers. 4 pytest tests. Deterministic run on the 12-subset landed H-null table: D=0 across S1/S2/S3; B ≥ 4× C; Oracle=1 on S1/S3 by design. LLM-gated run (~$5–15, 30–45min wall) in progress at the time of writing. Three deferred axes (robustness/OPR, inferential lift, mechanical H-workflow/H-spec/H-trigger, judge swap augmentation) tracked in `sim-bench-results.md` "Deferred axes".
