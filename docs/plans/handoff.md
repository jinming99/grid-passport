...
**Sprint 2026-04-20 ("demo-ready everything") in flight. Tracks 1 + 4 + 2.1a + 2.1b + 2.2 + 3.1 scaffolds all landed 2026-04-20; main run still deferred.** Sprint plan at `docs/plans/sprint-2026-04-20.md`: four tracks, ~4 weeks, four parallelizable streams. Track 1 (empirical evidence into `/about` §6) — OPR tables + P0.3 spot-check. Track 4 (derivation-correctness tests) — 48 new `node:test` units in `packages/core/src/{forecast,audit}.test.ts`; `pnpm core:test` runs 57 tests. Track 3.1 (utility binary) — `apps/utility/` workspace scaffolded; **gate 15 `canary:utility`** green. **Track 2 Interviewer (2.1a + 2.1b)** — fully wired end-to-end with real `claude -p` subprocess via Rust command; `canary:desktop` has 8 guards. **Track 2.2 Explainer** — read-scope Skill shipped: `.claude/skills/explainer/` (SKILL.md + ROLE_VOICES.md + 3 role-conditioned narration examples) + paired validator (`packages/agents/explainer/{src/validator.ts, scripts/validate_narration.ts}`) + mechanically-derived prompt-only baseline + substrate-metrics row (-96.2% upfront context saving · 4×never + 8×halt + 3×refuse clauses · 5 validator-refused contract-violation classes). **Research-thesis §6a metrics panel now spans 4 Skills across 2 domains × 2 contract axes** (write-scope × read-scope); §4 table Explainer row marked v0-shipped. Remaining open: 2.2-polish (desktop `ExplainerPanel` + `explainer_query` Rust command), 3.1-polish (utility full UX), 3.2 (self-review trust panel).
