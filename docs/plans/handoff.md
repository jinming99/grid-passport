...
**Sprint 2026-04-20 ("demo-ready everything") in flight. Tracks 1 + 4 landed 2026-04-20; Tracks 2.1a + 2.1b + 3.1 scaffolds landed 2026-04-20; main run still deferred.** Sprint plan at `docs/plans/sprint-2026-04-20.md`: four tracks, ~4 weeks, four parallelizable streams. Track 1 (empirical evidence into `/about` §6) — P0.1 CandidatePlan extraction + P0.3 different-family judge spot-check + live 12-cell re-score with robustness axis + `/about` §6 OPR tables. Track 4 (derivation-correctness tests) — 48 new `node:test` units in `packages/core/src/{forecast,audit}.test.ts`; `pnpm core:test` now runs 57 tests (↑ from 9). Track 3.1 (utility binary) — `apps/utility/` workspace scaffolded; **gate 15 `canary:utility`** green (write-scope import guard + verifier-wiring positive + sign/verify/tamper/reject e2e). **Track 2 (Interviewer Agent SDK) is fully wired end-to-end**: 2.1a shipped the transport seam + validator gating + IntakePanel; 2.1b shipped the real `claude -p` subprocess via Rust command (`interviewer_query`), ride-the-parent-session OAuth auth, tolerant JSON extraction, and a live-LLM smoke-test round-trip. `canary:desktop` now has 8 guards (was 3 at sprint start). Tracks 2.2 (Explainer Skill — unblocked now that the transport exists), 3.1-polish (utility full UX), and 3.2 (self-review trust panel) remain open.
...
- **Track 2.2 Explainer Skill v0** — unblocked, open. Same transport recipe: add an `explainer_query` Rust command + `.claude/skills/explainer/` with SKILL.md + ROLE_VOICES.md + 3 role-narration examples + paired validator. 2–3 d.
...
1. **Track 2.2 Explainer Skill v0** (~2–3 d) — now unblocked since 2.1b wired the transport. Pattern is a straight copy of the Interviewer: new `.claude/skills/explainer/` (SKILL.md declaring write-scope = "consumes only `ProjectedView`; never `CaseInput.privateProfile`; never writes any bucket" + `ROLE_VOICES.md` with applicant / utility / regulator voice specs + 3 canonical narration transcripts + paired validator that regex-asserts no raw-private-field matches in narration). Mechanically-derived prompt-only baseline via `pnpm agents:baseline`. Add an `explainer_query` Rust command mirroring `interviewer_query` — same envelope parsing; different system prompt. Adds row 3 to research-thesis §4 write-scope table.
