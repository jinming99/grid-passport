...
3. Replace the desktop app's fixture loader with `@tauri-apps/api/fs` for local file IO. **Input format: JSON** for v0 (matches the existing fixture schema, no migration); `.gpcase` thin wrapper deferred to Phase 5+ once we want OS-level file association.
...
**Decisions needed.** Whether v0 includes Interviewer/Cartographer agents (recommendation: ship without; agents land in #7/#8 right after).
