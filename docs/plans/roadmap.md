...
**Steps.**
1. Add `apps/desktop/` workspace with Tauri scaffold (Tauri 2.x; Rust core + Next.js frontend).
2. Hoist projection/forecaster/audit/policy into a shared package (`packages/core/`) so both web demo and desktop app import from the same source. (This refactor is the load-bearing piece — keep it surgical.)
3. Replace the desktop app's fixture loader with `@tauri-apps/api/fs` for local file IO.
4. Build the side-by-side review screen — three columns (applicant / utility / regulator) showing `ProjectedView` for each, with the BenefitPanel from #1 above each column.
5. Wire the "Export bundle" button (the bundle itself is just a JSON serialization at this stage — signing comes in #6).
6. macOS build via `pnpm tauri build`.
7. Run `pnpm typecheck && pnpm privacy:canary`. Add a `pnpm canary:desktop` that proves the desktop app uses the same projection code.
...
**Decisions needed.** Bundle file format (decision log #4); whether v0 includes Interviewer/Cartographer agents (recommendation: ship without; agents land in #7/#8 right after).
...
- **Derivation transparency review** — `flexibilityPassport.durationHoursMax = max(2, bessHours)` is identity for x≥2, leaking the private input through a derived field. Re-derive as bands. Audit other derived fields the same way.
...
| Date       | Item                                                                                              | Ref       |
| ---------- | ------------------------------------------------------------------------------------------------- | --------- |
| 2026-04-18 | `/about` story page — long-scroll render of `docs/story.md` (split by `##` section, custom SectionPlate per beat). Design pass via `/frontend-design:frontend-design` — classified-briefing × SCADA panel aesthetic: CAD-blueprint grid backdrop, scan-reveal hero, §NN mono markers, `mechanical canary · passing` telltale in §4, 3-card crew manifest in §9, "end of briefing" end-stamp. Cross-linked from landing + demo header. `@tailwindcss/typography` + `react-markdown` + `remark-gfm`. | this session |
...
2. **Privacy Benefit Panel — replace LeakCounter or sit alongside (#1)?** Recommendation: **replace**. The new panel is a superset — it counts hidden fields *and* shows the trust framing. Two side-by-side counters fragments the visual. But "replace" means a small migration in `DemoClient.tsx`. Confirm.
3. **Embed Privacy Benefit Panel on landing page (#3)?** Recommendation: **yes, as a static teaser** — show the metrics for a single canonical case (Owl Compute), with a "see the live panel for any case" link into the demo. Keeps the public-facing first impression dense.
4. **Tauri input file format (#4).** Recommendation: **JSON** for v0 — matches the existing fixture format, no schema migration needed. **`.gpcase`** file extension as a thin wrapper later (Phase 5+) once we want OS-level file association ("double-click to open in Grid Passport").
5. **Story page URL — `/about` vs `/story` (#5)?** Recommendation: **`/about`** — conventional, SEO-discoverable, matches what most users guess. `/story` reads as cute and might suggest "marketing copy" rather than substantive content. Confirm.
6. **Agent packaging route (#7, #8, #13).** Two options — both are Claude Agent Skills per the Anthropic docs; they differ in *where the skill runs*:
   - **(a) Local Agent Skill via the Claude Agent SDK** — SKILL.md sits at `packages/agents/<name>/SKILL.md`, distributed inside the Tauri bundle, invoked via the Agent SDK in-process. Pro: full network access from the skill's scripts, simplest local-first story. Con: requires the Agent SDK bundled into the Tauri app.
   - **(b) Hosted Agent Skill via the Claude API (`/v1/skills` upload)** — SKILL.md uploaded and invoked through the API's code-execution container. Pro: cleaner cross-surface deployment. Con: API runtime has no network, so Cartographer would need a workaround; raw inputs leave the local machine for the LLM call.
   - **Recommendation: (a) for the desktop app, (b) for the web demo's stage interactions.** Gives us the local-first trust story where it matters and a hosted demo path for the public landing page. Cross-surface skill drift is a real concern (per the Anthropic docs, "Custom Skills do not sync across surfaces") so we maintain SKILL.md as the single source of truth and copy/upload from there.
7. **Interviewer LLM call routing (#7).** Recommendation: **direct to Anthropic by default, with an optional `--llm-proxy` flag** for utilities that disallow third-party LLM calls. Default-friction stays low; sensitive deployments have an opt-out.
8. **LLM-judge rubric for human-preference eval (#14).** Needs Ming's input on what counts as "good" Explainer prose for each role. Should be done end of week 1 of the eval-harness sprint so Owner C/B isn't blocked. Recommendation: **3-axis 1–5 Likert per (case, role)** scoring: (a) does the prose answer the role's actual question; (b) does it stay within the projected view (no leakage of raw inputs); (c) is the tone calibrated (operational for utility, accountability for regulator, transparency for applicant). Plus a free-text "anything off?" field.
9. **First utility partner.** Out-of-scope for me — this is your relationship work, and Bhawuk's Dominion connection is a natural starting point. The Signed Bundle protocol (#6) is much more credible if it can be co-designed with one named utility's intake team. Worth flagging now.
