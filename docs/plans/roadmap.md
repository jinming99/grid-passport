...
- **Desktop Claude Agent SDK integration** — the loader produces `LoadedSkill[]` ready for SDK consumption, but the actual SDK wiring (LLM transport as Claude Code session or headless SDK, streaming, tool-invocation plumbing, clarify-question UX) is not yet implemented. Non-trivial; probably 3–5 days once started.
...
| 6a | Pilot wiring + 12-subset validation (`scripts/pilot.py --live` + ledger JSON persistence + committed manifest; S1/S2/S3 × A/B/C/D × seed=0, all 12 `ok`, ~85 min, ~$20–30) | ✅ done (`2d09b3d`) · directional signals surfaced (S2 C-vs-D round-count delta; S3 Kraken B no-meeting) |
| 6b | Seed expansion — 16+ more main-pilot runs to reach §9 seed coverage (n=5 on S1–S6) | pending · ~2.5–4 hrs + $40–80 |
| 6c | Fairness pilot — requires `runner.run()` `cartographer_mode_override` kwarg + flip `pilot.py --fairness` stub to real loop; 9 runs on S1/S3/S7 with C+D both-live | pending · ~30 min engine + 1–2 hrs runs |
| 6d | Scorer-over-ledger batch step — `scripts/score_ledgers.py` reads `results/pilot/transcripts/*.json` → writes `results/pilot/scores/*.json` + aggregated `summary.{json,md}`. **Do this before 6b spend** so scorer bugs surface on the 12 existing ledgers before we generate more. | pending · ~2–3 hrs engine + $20–50 LLM-gated scorers on 12-subset |
...
**Tests: 222 pass, ruff + pyright clean** as of `2d09b3d` (pilot.py wiring deliberately thin on unit tests — the meaningful test IS the live 12-subset run; manifest + per-run ledgers are the durable proof. Step 6d will add scorer-batch tests on FakeTransport-canned ledger fixtures).
