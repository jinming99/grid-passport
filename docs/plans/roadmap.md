...
## Between sprints — 2026-04-20 sprint closed

Sprint 2026-04-20 ("demo-ready everything") is **closed**. All four tracks + all polish items landed. Single-doc handoff at **`docs/plans/sprint-2026-04-20-brief.md`** (master student brief with four follow-on paths, useful invocations, honest-limits list). Track-by-track landing records stay intact at `docs/plans/sprint-2026-04-20.md`.

**Headline numbers at close:**
- 4 Skills × 2 domains × 2 contract axes on the substrate-metrics panel (uniform −93.8% to −96.4% upfront context saving).
- `canary:desktop` 3 → 12 guards; `canary:utility` new (gate 15); `core:test` 9 → 57 tests.
- All 15 gates green.
- Two-binary demo story works: applicant Tauri → signed bundle → utility Tauri verifies + renders, no network between.

**Not yet in a new sprint (awaiting scope):**
- Sim-bench main run (455 runs, $400–1200).
- P0.2 ledger metadata extension (unblocks §8d H-workflow / H-spec / H-trigger).
- Student track P1.2 / P1.3 / P1.4 (specs at `docs/evals/specs/`).
- #2 open-source repo flip (parked on go/no-go).
- Windows MSI / Linux AppImage builds for both Tauri binaries.
- Dominion onboarding handshake (gated on Bhawuk intro).
- Prod-launch `claude`-CLI PATH expansion for `.app` bundles.

See the brief's "Four student paths" section for re-packaged routes into these items.
...
- ~~**Desktop Interviewer transport architecture (Track 2.1a)**~~ — ✅ landed 2026-04-20. Pure validator extracted to `@grid-passport/agents/interviewer/validator`; transport seam + Fake default + IntakePanel React component.
- ~~**Real Claude Agent SDK transport (Track 2.1b)**~~ — ✅ landed 2026-04-20. Path (b) chosen: Rust Tauri command `interviewer_query` at `apps/desktop/src-tauri/src/interviewer.rs` shells out to `claude -p --output-format json --system-prompt <skill>` with the parent Claude Code session's OAuth env inherited. Frontend dynamic-imports `@tauri-apps/api/core::invoke`, parses the envelope, tolerant-extracts first JSON object, runs validator, assembles CaseInput. Live-smoke-tested end-to-end. `canary:desktop` now has 8 deterministic guards.
...
| Date       | Item                                                                                              | Ref       |
| ---------- | ------------------------------------------------------------------------------------------------- | --------- |
| 2026-04-20 | **Sprint 2026-04-20 closed — all four tracks + polish landed.** Single-doc student brief at `docs/plans/sprint-2026-04-20-brief.md`. **Track 1**: P0.1 CandidatePlan extraction + P0.3 cross-family judge spot-check + 12-cell re-score + OPR tables in `/about §6`. **Track 2**: 2.1a Interviewer transport seam + `IntakePanel` + validator-gated emission; 2.1b real `claude -p` subprocess via Rust IPC (`interviewer.rs` + dynamic invoke); 2.2 Explainer Skill v0 (SKILL.md + ROLE_VOICES.md + 3 role-narration examples + paired validator + mechanically-derived baseline); 2.2-polish `ExplainerPanel` + `explainer_query` Rust command. **Track 3**: 3.1 scaffold (new `apps/utility/` workspace + gate 15 `canary:utility` enforcing import-graph write-scope); 3.1-polish real drop-zone + `UtilityProjection` renderer + trust-claim stamp; 3.2 `TrustPanel` live-state chrome in desktop. **Track 4**: `forecast.test.ts` (30) + `audit.test.ts` (18) unit tests — `core:test` 9 → 57. **Research-thesis §6a** now spans 4 Skills × 2 domains × 2 contract axes with uniform −93.8% to −96.4% upfront context saving. `canary:desktop` 3 → 12 guards; `canary:utility` new; 15 gates green. | `3cfac7d` `1ffa73c` `625f7cf` `63ff7b7` `931f039` `2513690` `3c0735f` `c8f0ea4` `9a0a735` `18491af` |
