...
| -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| Raw competitive fields exposed per request   | 6 of 18 (workload mix, schedule confidence, redundancy %, BESS sizing, backup gen sizing, flex %)                | **0 of 18**                                                         | Field count from this repo's `apps/web/lib/types.ts` — directly inspectable.                          |
| Decision-relevant signal preserved           | All of it (utility makes decision from raw fields).                                                              | **All of it** (7 derived proofs cover the same decision surface).   | `apps/web/lib/forecast.ts` — derivation maintains decision parity by construction.                    |
